...
# For phytoplankton
# taxa_meta_concentration <- taxa_meta %>%
#   mutate(across(c(Ceratium,Chaetoceros,Diatoms,Diatoms2,Guinardia,Neocalyptrella,Trichodesmium), ~ ./total_vol_sampled * 1e6)) %>%
#   select(Station, dec_lat, dec_lon, year, month, date, Avg.chl.a..ug.L.,
#          temp..degC., salinity,total_vol_sampled,
#          Ceratium,Chaetoceros,Diatoms,Diatoms2,Guinardia,Neocalyptrella,Trichodesmium) %>%
#   filter(!is.na(total_vol_sampled))
# 
# # Transform taxa_meta_concentration to long format
# taxa_meta_long <- taxa_meta_concentration %>%
#   pivot_longer(cols = c(Ceratium,Chaetoceros,Diatoms,Diatoms2,Guinardia,
#                         Neocalyptrella,Trichodesmium),
#                names_to = "species", values_to = "species_concentration")
...
#   filter(Station %in% station_list_cal)
...
custom_pal_hex2 <- c('darkturquoise','red','#4daf4a','slategray4','orange','dodgerblue','purple', "yellow")
...
taxa_concentration_avg <- concentration_df %>%
  group_by(line_id) %>%
  mutate(weight = Larvaceans / sum(Larvaceans, na.rm = TRUE)) %>%
  summarise(longitude = weighted.mean(dec_lon, w = weight, na.rm = TRUE),
            latitude = weighted.mean(dec_lat, w = weight, na.rm = TRUE),
            sel_taxa_mean = mean(Larvaceans, na.rm = TRUE),
            sel_taxa_sd = sd(Larvaceans, na.rm = TRUE))
