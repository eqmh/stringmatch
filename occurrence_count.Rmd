...
dt <- data.frame(
  ...
  sel_param = log(filtered_taxa_avg$sel_taxa_mean))
    
occ_map <- basemap(limits = c(-84, -79.5, 24, 28.5), bathymetry = TRUE) +
  geom_point(data = dt, aes(x = lon, y = lat, size = sel_param), fill = "hotpink", color = "hotpink") +
  scale_size_continuous(name = "log(counts)", limits = log(c(0.1, 20)), range = c(0.1, 10)) +
  theme_minimal() 
occ_map
