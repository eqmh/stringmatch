...
# for (class_name in class_names) {
    ...
    taxa_concentration_avg <- concentration_df %>%
      group_by(line_id) %>%
      mutate(weight = get(class_names) / sum(get(class_names), na.rm = TRUE)) %>%
      summarise(longitude = weighted.mean(dec_lon, w = weight, na.rm = TRUE),
                latitude = weighted.mean(dec_lat, w = weight, na.rm = TRUE),
                sel_taxa_mean = mean(get(class_names), na.rm = TRUE),
                sel_taxa_sd = sd(get(class_names), na.rm = TRUE))
    ...
    theme_minimal() +
    ggtitle(paste(class_names))
      
    # Optionally, save each map as an image
    ggsave(paste0("concentration_map_", class_names, ".png"), plot = concentration_map, width = 8, height = 6)
