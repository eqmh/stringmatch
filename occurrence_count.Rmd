...
# # Percent of sampling events without a seascape class match
...
percent_match <- round(seascape_match/sampling_events*100, digits = 0)

sampling_events_total <- paste("Total number of sampling events (CTD casts):", nrow(taxa_meta))
matched_casts <- paste("Percent of sampling events with seascape match:", round(seascape_total/nrow(taxa_meta)*100, digits=0), "%")
sampling_success <- paste("Percent of sampling events with images:", round(sampling_events/nrow(taxa_meta)*100, digits=0), "%")
match_stat <- paste("Percent of sampling events with a seascape match:", percent_match, "%")
...
avg_stackplot <- ggplot(avg_concentration_per_class, aes(x = X8.day.seascapes, y = mean_concentration, fill = species)) +
  ...
  theme(axis.text.x = element_text(size = font_size, color = "black"),
        ...
        axis.title.y = element_text(size = font_size, color = "black", margin = margin(r = 21))) + # margin(r = 21) used to separate the title from the axis for the zooplankton plot only
  theme(axis.line = element_line(color = "black")) +
...
font_size = 36
pp <- df_filtered %>%
  ggplot( aes(x=X8.day.seascapes, y=NH4...uM., fill=X8.day.seascapes)) +
    geom_boxplot() +
...
ggsave("ammonium.svg", plot = pp, width = 14, height = 8, device = "svg")
```
...
# # Mean plankton densities per seascape using station data (same results as above)
```{r}
...
print(summary_tbl_concentrations_seascapes)
...
avg_stackplot2 <- ggplot(summary_tbl_filtered, aes(x = X8.day.seascapes, y = mean_species_concentration, fill = species)) +
  geom_bar(stat = "identity", width = 0.75) +
  # scale_fill_manual(values = spp_colors_zoopl,
  # labels = spp_labels_zoopl) +
  scale_fill_manual(values = spp_colors_phyto,
...
# # Create plots showing coefficient of variation of plankton abundances per seascape
```{r}
...
# ggsave("phyto_cv.svg", plot = heatmap, width = 16, height = 7.25, device = "svg")
# ggsave("zoopl_cv.svg", plot = heatmap, width = 16, height = 8, device = "svg")
```


# # Create plots showing coefficient of variation of plankton relative proportion per seascape
```{r}
...
# ggsave("phyto_cv_relabund.svg", plot = heatmap, width = 16, height = 7.25, device = "svg")
# ggsave("zoopl_cv_relabund.svg", plot = heatmap, width = 16, height = 8, device = "svg")
```
