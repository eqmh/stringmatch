...
# # Calculate plankton time series per station
```{r}
...
taxa_meta_concentration <- taxa_meta %>%
  mutate(across(c(Acantharea, 
                  ...
                  Larvaceans, 
                  Polychaetes, 
                  Pteropods), ~ ./total_vol_sampled * 1e6)) %>%
  select(Station, dec_lat, dec_lon, date, Avg.chl.a..ug.L., temp..degC., salinity,
         Acantharea, 
         Copepods, 
         Echinoderms, 
         Jellies, 
...
# Remove rows with NA values under specified columns
taxa_meta_concentration <- taxa_meta_concentration %>%
  na.omit(select(taxa_meta_concentration, 
                 Acantharea, 
                 Copepods, 
                 Echinoderms, 
                 Jellies, 
...
taxa_meta_long <- taxa_meta_concentration %>%
  pivot_longer(cols = c(Acantharea, Copepods, Echinoderms, Jellies, Larvaceans, Polychaetes, Pteropods),
               names_to = "species", values_to = "species_concentration")

# Filter the data for Stations 68, 65, and 65
filtered_taxa_meta_long <- taxa_meta_long %>%
  filter(Station %in% c("TB1", "TB2", "TB3", "TB4", "TB5", "TB10"))
...
# Filter summary_data for copepods only
# summary_data <- summary_data %>%
#   filter(species == "Copepods")

# Plot time series with mean and standard deviation
ggplot(summary_data, aes(x = date, y = mean_concentration, color = species)) +
  geom_bar(stat = "identity", alpha=0.7) +
  geom_col(width = 2, position = position_dodge(0.7)) +
  geom_errorbar(aes(ymin = mean_concentration - sd_concentration, ymax = mean_concentration + sd_concentration), width = 0.2) +
  labs(x = "Date", y = "Mean Species Concentration", color = "Species") +
  theme_minimal()
...
# Calculate speciess per cubic meter
...
concentration_percent_stackplot <- ggplot(spp_concentration_percent, aes(x = X8.day.seascapes, y = species_percentage, fill = species)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = custom_pal_hex2) +
  labs(x = "Seascape class", y = "Species concentration (%)") +
  theme_minimal() +
