...
for (i in 1:(length(dt_list))) {
  ...
  ostra_subset <- subset(class.Ostracods, date >= dt_list[i]-start & date < dt_list[i]+stop) 
    ostra_count <- nrow(ostra_subset)
...
#   mutate(across(c(Acantharea,Copepods,Echinoderms,Jellies,Larvaceans,Chaetognaths,
#                   Polychaetes,Pteropods), ~ ./total_vol_sampled * 1e6)) %>%
...
#   pivot_longer(cols = c(Acantharea, Copepods, Echinoderms, Jellies, Larvaceans,
#                         Chaetognaths, Polychaetes, Pteropods),
#                names_to = "species", values_to = "species_concentration")

# # For phytoplankton
taxa_meta_concentration <- taxa_meta %>%
  mutate(across(c(Centric,Ceratium,Chaetoceros,Diatoms,Diatoms2,Guinardia,Neocalyptrella,
                  Noctiluca,Trichodesmium), ~ ./total_vol_sampled * 1e6)) %>%
  select(Station, dec_lat, dec_lon, year, month, date, Avg.chl.a..ug.L.,
         temp..degC., salinity,total_vol_sampled,
         Centric,Ceratium,Chaetoceros,Diatoms,Diatoms2,Guinardia,Neocalyptrella,
         Noctiluca,Trichodesmium) %>%
  filter(!is.na(total_vol_sampled))

# # Transform taxa_meta_concentration to long format
taxa_meta_long <- taxa_meta_concentration %>%
  pivot_longer(cols = c(Centric,Ceratium,Chaetoceros,Diatoms,Diatoms2,
                        Guinardia,Neocalyptrella,Noctiluca,Trichodesmium),
               names_to = "species", values_to = "species_concentration")

# Filter the data for Stations
station_list_fk <- c("WS","21/LK","MR","16","18","10","12","9.5","9","7","2","KW1","KW2","KW4")
...
custom_pal_hex2 <- c('darkturquoise','red','#4daf4a','slategray4','orange','dodgerblue','purple', "yellow","pink")
...
concentration_ts <- ggplot(aggregated_concentration, aes(x = year_month)) +
  ...
  labs(title = "Total Phytoplankton Concentration and Temperature Time Series",
       x = "Date",
...
pp <- df_filtered %>%
  ggplot( aes(x=X8.day.seascapes, y=PO4...uM., fill=X8.day.seascapes)) +
    ...
    geom_jitter(color="grey", size=0.8, alpha=0.7) +
...
# Map overall mean concentration values of selected taxa 
```{r}
library(ggOceanMaps)
library(leaflet)
...
taxa_concentration_avg <- concentration_df %>%
  group_by(line_id) %>%
  mutate(weight = Ceratium / sum(Ceratium, na.rm = TRUE)) %>%
  summarise(longitude = weighted.mean(dec_lon, w = weight, na.rm = TRUE),
            latitude = weighted.mean(dec_lat, w = weight, na.rm = TRUE),
            sel_taxa_mean = mean(Ceratium, na.rm = TRUE),
            sel_taxa_sd = sd(Ceratium, na.rm = TRUE))
...
concentration_map <- basemap(limits = c(-84, -79.5, 24, 28.5), bathymetry = TRUE) +
  ...
  geom_point(data = taxa_meta_concentration, aes(x = dec_lon, y = dec_lat), color = "black", shape = 3, size = 1.5) +
