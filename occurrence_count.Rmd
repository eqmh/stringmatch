...
df_sum_abund_filt <- df_sum_abund[df_sum_abund$Species == "Acantharea",]
# Filter the data to exclude specified seascapes
...
occ_map <- basemap(limits = c(-84, -79.5, 24, 28.5), bathymetry = TRUE) +
  geom_point(data = dt, aes(x = lon, y = lat, size = sel_param), fill = "red", color = "red") +
  scale_size_continuous(name = "log(counts)", limits = log(c(0.1, 30)), range = c(2, 10)) +
  theme_minimal() 
occ_map
