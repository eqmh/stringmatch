...
summary_cv_table <- coeff_var %>%
  group_by(X8.day.seascapes) %>%
  summarise(
    mean_coef_var = mean(coef_var, na.rm = TRUE),
    sd_coef_var = sd(coef_var, na.rm = TRUE)
  ) %>%
  ungroup()

summary_cv_table$seascape_class <- factor(summary_cv_table$X8.day.seascapes,
                                   levels = c(3, 11, 13, 15, 21, 27),
                                   labels = c("TST", "TSU", "SGMI", "TS", "WBHN", "HE"))

print(summary_cv_table)

total_average_rows <- summary_cv_table %>%
  mutate(
    species = "total average",
    coef_var = mean_coef_var
  ) %>%
  select(seascape_class, species, coef_var)

coeff_var_extended <- bind_rows(coeff_var, total_average_rows)

coeff_var_extended$species <- factor(
  coeff_var_extended$species,
  levels = c(setdiff(unique(coeff_var_extended$species), "total average"), "total average")
) 
...
#   "total average" = "Total average"  # preserve this row label
# )

species_labels <- c(
  "Acantharea" = "Rhizaria spp",
...
coeff_var_extended <- coeff_var_extended %>%
...
coeff_var_extended$species <- factor(coeff_var_extended$species, levels = rev(species_order))

heatmap <- ggplot(coeff_var_extended, aes(x = seascape_class, y = species, fill = coef_var)) +
  geom_tile(color = "white") +
  # scale_fill_gradient(low = "white", high = "forestgreen", name = "CV") + # for phyto
  scale_fill_gradient(low = "white", high = "darkred", name = "CV") + # for zoopl
  theme_minimal() +
  labs(x = NULL, y = NULL) +
  theme(
...
# ggsave("phyto_cv.svg", plot = heatmap_phyto, width = 14, height = 8, device = "svg")
ggsave("zoopl_cv.svg", plot = heatmap, width = 14, height = 8, device = "svg")
```
