...
#####################################################################################################################
# This section looks at time stamp mismatches between CTD and CPICS data
...
# Calculate the time differences in minutes between consecutive dates is selected class. 
# This finds time stamps for individual casts based on the actual CPICS data
time_diffs <- difftime(class.Copepods$date[-1], class.Copepods$date[-nrow(class.Copepods)], units = "mins")

# Find the indices where the time difference is greater than 20 minutes
indices <- which(time_diffs > 20)
timestamp_list <- class.Copepods$date[indices]
...
taxa_meta_concentration <- taxa_meta %>%
  mutate(across(c(Acantharea,Chaetognaths,Ostracods,Copepods,Decapods,Echinoderms,Jellies,
    Larvaceans,Polychaetes,Pteropods), ~ ./total_vol_sampled * 1e6)) %>%
  select(Station, dec_lat, dec_lon, year, month, date, Avg.chl.a..ug.L.,
         ...
         Jellies,Larvaceans,Polychaetes,Pteropods) %>%
...
taxa_meta_long <- taxa_meta_concentration %>%
  pivot_longer(cols = c(Acantharea, Chaetognaths,Ostracods,Copepods,Decapods,
                        Echinoderms, Jellies, Larvaceans,Polychaetes, Pteropods),
