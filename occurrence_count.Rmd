...
for (i in 1:(length(dt_list))) {
  ...
  result <- data.frame(date = dt_list[i], 
                       ...
                       Neocalyptrella_spp = neocal_count,
...
df_sum_abund_filt <- df_sum_abund[df_sum_abund$Species == "Copepods",]
# Filter the data to exclude specified seascapes
...
taxa_avg <- filtered_taxa_meta %>%
  group_by(line_id) %>%
  summarise(longitude = mean(dec_lon),
            latitude = mean(dec_lat),
            sel_taxa_mean = mean(Echinoderms, na.rm = TRUE))

# # Filter out values less than 1
filtered_taxa_avg <- taxa_avg %>%
  filter(sel_taxa_mean >= 0.1)

# Assuming 'filtered_taxa_avg' is your data frame
...
dt <- data.frame(lon = filtered_taxa_avg$longitude, lat = filtered_taxa_avg$latitude, sel_param = filtered_taxa_avg$sel_taxa_mean)
    
occ_map <- basemap(limits = c(-84, -79.5, 24, 28.5), bathymetry = TRUE) +
  geom_point(data = dt, aes(x = lon, y = lat, size = sel_param), fill = "gold1", color = "gold1") +
  scale_size_continuous(name = "Symbol Size", range = c(1, 10)) +
  theme_minimal() 
occ_map
