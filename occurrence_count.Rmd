...
df_summary_relabund <- taxa_meta_long_all_seascapes %>%
    filter(species %in% c("Centric","Ceratium","Chaetoceros","Chain2","Chain3","Guinardia","Neocalyptrella", "Noctiluca","Tricho")) %>%
  group_by(year, month, X8.day.seascapes) %>%
  mutate(class_total = sum(`species_concentration`, na.rm = TRUE)) %>%
  group_by(year, month, X8.day.seascapes, species) %>%
  summarise(
    abundance = sum(`species_concentration`, na.rm = TRUE),
    class_total = first(class_total),
    rel_abundance = abundance / class_total
  ) %>%
  select(year, month, X8.day.seascapes, species, rel_abundance) %>%
  ungroup()

# Calculate relative abundance CV per seascape
cv_summary_relabund <- df_summary_relabund %>%
  group_by(X8.day.seascapes, species) %>%
  summarise(
    mean_relabund = mean(rel_abundance[rel_abundance != 0], na.rm = TRUE),
    sd_relabund = sd(rel_abundance[rel_abundance != 0], na.rm = TRUE),
